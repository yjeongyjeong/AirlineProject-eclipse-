...
<form id="myForm" action="UserServlet" method="post" name="frm" style="text-align: center;">
	...
	<c:choose>
		<c:when test="${result == '1' }">
			<h4 align="center" style="color: gray"><br>입력하신 정보를 확인중입니다.<br>창을 닫지 마세요.</h4>
			<input type="hidden" name="command" value="auth_email_send">
			<input type="hidden" name="userid" value="${userid}"> <!-- get방식(url)로 넘어옴 -->
			<input type="hidden" name="email" value="${email}"> <!-- get방식(url)로 넘어옴 -->
			<script type="text/javascript">opener.close();</script>
		</c:when>
		<c:otherwise>
			<h4> 입력하신 정보가 올바르지 않습니다.</h4>
