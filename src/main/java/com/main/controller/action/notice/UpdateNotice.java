...
@MultipartConfig(location = "/noticeFileUpload", // 업로드된 파일을 임시로 저장할 디렉토리 경로
...
public class UpdateNotice implements Action {
	...
	public void execute(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		MultipartRequest multi = new MultipartRequest(request, uploadPath, 1024 * 1024 * 50, // 최대 파일 크기 (50MB)
				"UTF-8", new DefaultFileRenamePolicy() // 파일명 중복 처리
		);
		...
		fVo.setBoardnum(Integer.parseInt(multi.getParameter("boardnum")));
		fVo.setOrigin(originalFileName);
		fVo.setUuid(fileName);
		fVo.setFileurl(fileSystemName);
		
		vo.setBoardnum(Integer.parseInt(multi.getParameter("boardnum")));
		...
		vo.setModifydate(multi.getParameter("modifydate"));
		nDao.updateNoticeFile(fVo);
		// 데이터베이스에 게시물 정보 및 파일 정보 저장
		int result = nDao.updateNoticeBoard(vo);
		System.out.println("마지막 : " + fVo);
		System.out.println("마지막 : " +vo);
