...
public class LoginQna implements Action {
	...
	public void execute(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		String url = "BoardServlet?command=qna_list";
		QnaDAO qdao = QnaDAO.getInstance();
		// result가 1이면 로그인성공, 0이면 암호 틀림, -1이면 아이디 틀림
		int result = qdao.userCheck(username, pass);
		System.out.println("로그인 : " + result);
		if (result == 1) {
			UserVO vo = qdao.getMember(username);
			HttpSession session = request.getSession();
			session.setAttribute("username", vo);
			System.out.println("listuser : "+vo );
			url = "BoardServlet?command=qna_list";
		} else if (result == 0) {
